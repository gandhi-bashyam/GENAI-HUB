...
class RAGPipeline:
    def __init__(
        ...
        prompt_version="v1",
        top_k=5,
        grounding_threshold=0.6  # ✅ Default grounding threshold
    ):
    ...
    def _retrieve(self, query):
        ...
        if cached_docs:
            ...
            if hasattr(self.retriever, "retrieve_async"):
                docs = asyncio.run(self.retriever.retrieve_async(query, top_k=self.top_k))
            else:
    ...
    def _multi_retrieve(self, queries):
        all_results = [self._retrieve(q) for q in queries]
    ...
    def _prioritize_sources(self, docs, query):
        ...
        return sorted(
            docs,
            key=lambda x: (priority.get(x.get("metadata", {}).get("source_type"), 0), x.get("score", 0)),
            reverse=True
        )
...
"""
    ...
    def _generate(self, query, prompt):
        ...
        print(f"⏱️ LLM time: {self.metrics['llm_time']:.2f}s")
        return response
